...
- Make rules deterministic and unit-testable with Python standard library plus PyYAML for Kubernetes YAML parsing.
...
The repository currently has no `pyproject.toml`, `requirements.txt`, or lockfile. v0.4.0 should keep that lightweight structure and use `uv --with pyyaml` for commands that import PyYAML.

User-facing commands that run the static review helper should include PyYAML explicitly:

```powershell
uv run --with pyyaml python scripts/opswiki_static_review.py --input examples/sample-infra --output examples/sample-output/static-review.md --project-name sample-infra
```

Test commands for the static review helper should also include PyYAML explicitly:
...
The script should produce a clear error if PyYAML is unavailable and Kubernetes YAML files need to be scanned. Terraform-only scans should not require PyYAML.
...
Severity should be deterministic and rule-based:

| Rule ID | Severity | Rationale |
| --- | --- | --- |
| `TF_PUBLIC_INGRESS` | `high` | Public ingress can expose services broadly and should be intentionally justified. |
| `TF_WILDCARD_IAM_ACTION` | `high` | Wildcard IAM actions can materially increase blast radius. |
| `TF_WILDCARD_IAM_RESOURCE` | `high` | Wildcard IAM resources can materially increase blast radius. |
| `TF_SENSITIVE_OUTPUT` | `medium` | Sensitive-looking outputs may leak secrets unless marked sensitive. |
| `TF_MISSING_TAGS` | `low` | Missing tags are usually operability and ownership risks, but provider defaults or modules may add tags elsewhere. |
| `K8S_PRIVILEGED_CONTAINER` | `high` | Privileged containers bypass important workload isolation controls. |
| `K8S_HOST_PATH_VOLUME` | `high` | Host path mounts can expose node filesystems to workloads. |
| `K8S_HOST_NETWORK` | `high` | Host networking weakens pod isolation and can bypass expected network controls. |
| `K8S_LATEST_IMAGE_TAG` | `medium` | Mutable image tags reduce deploy reproducibility. |
| `K8S_MISSING_RESOURCE_REQUESTS` | `medium` | Missing requests reduce scheduling and capacity predictability. |
| `K8S_MISSING_RESOURCE_LIMITS` | `medium` | Missing limits can increase noisy-neighbor and runaway resource risk. |
| `K8S_MISSING_READINESS_PROBE` | `medium` | Missing readiness probes can send traffic to containers before they are ready. |
| `K8S_MISSING_LIVENESS_PROBE` | `low` | Missing liveness probes are often a reliability concern, but some workloads intentionally avoid them. |
| `K8S_DEFAULT_NAMESPACE` | `low` | Missing namespace can be intentional in templates, but it is an environment ambiguity. |
| `K8S_SERVICE_SELECTOR_WITHOUT_MATCHING_WORKLOAD` | `low` | The matching workload may be outside the scanned files or generated by tooling. |
| `K8S_YAML_PARSE_ERROR` | `medium` | Invalid YAML blocks reliable manifest review. |
...
- `TF_MISSING_TAGS`: flag only these standalone AWS resource blocks when no `tags` block or `tags =` assignment appears inside the same resource block: `aws_instance`, `aws_security_group`, `aws_lb`, `aws_db_instance`, `aws_s3_bucket`, `aws_eks_cluster`, and `aws_lambda_function`.
...
Terraform rule wording must avoid claiming final policy failure when text-only evidence is incomplete. For example, `TF_MISSING_TAGS` should say "No tags were found in this resource block" rather than "Resource is untagged across all environments."
...
- `K8S_LATEST_IMAGE_TAG`: flag containers using `:latest` or no explicit tag. Do not flag digest-pinned images such as `repo/app@sha256:<digest>`. Treat registry ports such as `localhost:5000/app:1.0` as valid tagged images.
...
- `K8S_SERVICE_SELECTOR_WITHOUT_MATCHING_WORKLOAD`: flag Service selectors when no scanned workload labels appear to match, and word the finding as "No matching workload labels were found in scanned files."

The scanner should handle multi-document YAML. It should ignore empty YAML documents.

Kubernetes rule wording must distinguish static review evidence from cluster truth. Findings should say "in scanned files" when the matching workload, namespace, or policy context could exist outside the input directory.
...
4. Load Kubernetes YAML files with PyYAML when Kubernetes scanning is enabled.
...
## Backlog Outside v0.4.0

- Improve v0.3 runbook generation by mapping source note sections such as `Common Causes` into runbook diagnosis content or a dedicated generated section.
- Add optional dependency management files only if future scripts require more than a small `uv --with ...` dependency set.
